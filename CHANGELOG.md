...
- **大纲（WYSIWYG）**：点击右侧大纲可滚动到对应标题；修复 `headingIndex` 遍历覆盖导致总是滚到底部、焦点在 TOC 时 PM 不滚动、flex 容器无法滚动等问题。
- **搜索替换**：非正则模式下「全部替换」按字面量处理（如 `$100`），避免被当作替换模式。
- **文件关联**：双击 `.md` / `.mkd` 等在单实例下正确打开目标文档并授权目录。

### 变更

- 方案文档：新增 `10`、`11`、`12` 号修复说明。
